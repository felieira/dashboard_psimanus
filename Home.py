...
def check_password():
    ...
    def password_entered():
        # Definir senhas padrão caso não existam nos segredos
        passwords = st.secrets.get("passwords", {"admin": "admin", "usuario1": "senha1"})
        
        if st.session_state["username"] in passwords and st.session_state["password"] == passwords[st.session_state["username"]]:
            st.session_state["password_correct"] = True
            del st.session_state["password"]  # Não armazene a senha
            del st.session_state["username"]  # Não armazene o nome de usuário
        else:
            st.session_state["password_correct"] = False

    if "password_correct" not in st.session_state:
        # Primeiro acesso, mostrar formulário de login
        st.text_input("Usuário", key="username")
        st.text_input("Senha", type="password", key="password")
        st.button("Entrar", on_click=password_entered)
        return False
    elif not st.session_state["password_correct"]:
        # Senha incorreta, mostrar formulário de login novamente
        st.text_input("Usuário", key="username")
        st.text_input("Senha", type="password", key="password")
        st.button("Entrar", on_click=password_entered)
...
def main():
    ...
    #     st.stop()  # Não continuar se a senha estiver incorreta
    ...
    try:
        ...
        # Se nenhuma credencial foi obtida, mostrar erro e parar
        if creds is None:
